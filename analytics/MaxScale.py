...
def socketStartForQStats(argv):
    host=argv[3]
    print "opening connection....for queue stats%s"% argv[1],
    port=50005
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    print "host:"+str(host)
    s.connect((host, port))
...
def socketStartForTrafficStats(argv):
    host=argv[3]
    print "opening connection....for traffic stats %s"% argv[1],
    port=50006
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    print "host:"+str(host)
    s.connect((host, port))
...
def sendPacket(s, packet, interface, argv):
    ...
    #print 'Printing Packet Details:',sp
    '''sending package'''
    try:
	s.sendall(buffer+sp)
    ...
    for process in range(0,2):
        ...
        if child:
            ...
            if process==0:
                ...
                s = socketStartForTrafficStats(argv)
		deviceInfoTransmission= deviceInfo(s,device_Name_Interface, deviceSerialNum,model, argv)
...
def main(argv):
    ...
    port = 50005
    print "Transmitting for device %s=%s:xe-0/0/" % (device_Name,  deviceSerialNum)
